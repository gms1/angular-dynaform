...
@Component({
  ...
  template: `
    <div [formGroup]="model.ngGroup" [hidden]="model.hidden">
      <fieldset [id]="model.id" ngDefaultControl adfHTMLDomElement [ngClass]="model.css.control">
        <legend
        ...
        <label *ngFor="let opt of model.local.valueOptions">
          <input [formControlName]="model.key" type="radio" [value]="opt.value" />
...
export class BasicRadioGroupComponent extends DynamicFormControlComponent<ValueModel> {
  model!: ValueModel;
